...
class PDF extends pdflib {
	...
	public function __construct ($file = 'test.pdf') {
		...
		$this->file = $file;
		$this->fontname = 'Helvetica'; #Fraktur BT'; #Frutiger LT Std';
		...
		$this->font = $this->load_font($this->fontname, 'host', '');
		$this->boldfont = $this->load_font($this->fontname, 'host', 'fontstyle=bold');
		$this->italicfont = $this->load_font($this->fontname, 'host', 'fontstyle=italic');
	...
	function sendMail ($to, $text, $subject = "Deputatsverwaltung") {
		
		// Daten lesen
		$data = chunk_split(base64_encode($this->get_buffer()));
...
#		$to = "rainer.sawatzki@haw-hamburg.de";
		...
		$headers = "From: Martin.Holle@haw-hamburg.de\n" .
			"MIME-Version: 1.0\n" . 
		...
		foreach ($this->getInhalt() as $line) {
			...
			*/
			...
			do {
				...
				error_log($printable->getHandle() . " -> $result\n");
			...
			if ($result != "_stop" && $result != '_pagebreak') {
				if ($result == "_error") {
					die ("Error when placing printable: " . PDF_get_errmsg($printable));
				} else {
...
class PDF_Body extends PDF_Template {
	...
	function  __construct(PDF_Document $document, $optlist = "") {
		parent::__construct($document, $document->getBodywidth(), $document->getBodyheight());
